...
グローバル変数報はキーをグローバル変数名、値を変数情報としたハッシュテーブルです。
    
ローカル変数情報へのアクセスは以下の手順で行います。
1. キーをローカル変数名、値をブロックの深さとした、  
ローカル変数ブロックハッシュテーブルを使用して、  
その変数のブロック深さを取得します。  
  
2. ブロック深さをインデックスとする、  
ブロック別ローカル変数定義テーブルベクトルにアクセスして、  
ブロック別ローカル変数定義テーブルを取り出します。

3. ブロック別ローカル変数定義テーブルから目的のローカル変数の情報を取得します。  
ブロック別ローカル変数定義テーブルは、  
ブロックごとのローカル変数定義ハッシュテーブルであり、    
そのブロックで宣言されたローカル変数名をキーとし、  
スタックフレームからのオフセットと型情報を値として持ちます。  

(もしより良い実装方法があれば教えて下さい)
